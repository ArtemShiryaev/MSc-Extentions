...
N = 1; % Define the number of iterations
...
% initial conditions
th0 = 0.01; Dth0 = 0;
new_step_size = 4;       % this variable defines new step size for simulated data resampling procedure
steady_state_behavior = 45; % This variables controls how many seconds we should consider, starting from the end
% of the experiment. E.g. steady_state_behavior = 45 implies only
% considering the last 45 seconds of the experiment. Ignoring the rest, as
% it is considered a transitional period into steady-state behaviour.
...
Omega_opt_eq100 = zeros(N,1);
Alpha_Values = zeros(N,2);
...
for iter = 1:N
    ...
    % From prev script
    Mass_alpha2 = Mass_alpha*(iter -1); % 
    Length_alpha2 = Length_alpha*(iter -1);
    Alpha_Values(iter,:) = [Mass_alpha2 Length_alpha2];
    p1_2 = M*L^2 + Jc + Mass_alpha*Length_alpha^2 ; p2_2 = M*L*g + Mass_alpha*g*Length_alpha ; p3_2 = B;

    sys2 = tf(p3_2,[p1_2 Kf p2_2]);
    ...
    seed_rand = 222; % internal parameter
    simIn = Simulink.SimulationInput("Simulink"); 
    number_of_recorded_experiments = length(scales);
    
    % Set up
    number_of_iterations = 1; % spread around of the point estimates
    ...
    for NN = 1:number_of_recorded_experiments
        A_in2 = scales(NN); 
        Om_in2 = input_freq(NN); 
        for iteration = 1:number_of_iterations
            %% Step 2: run synthetic experiment NN for generating the  data 
            out = sim(simIn);
            t = out.tout;   % time
            u = out.u.Data; % input e.g. sin
            q = out.q.Data; % angle
            seed_rand = 222*iteration; % use wish to conduct 'many' estimates of same MLE estimate

            %% Step 3: Visualize Input signal
            u_max = max(u); 
            ...
            q_steady_state_r = q_resampled( length(q_resampled)-floor(steady_state_behavior/dt_r):end );
            t_steady_state_r = t_resampled( length(q_resampled)-floor(steady_state_behavior/dt_r):end );

            q_steady_state = q_with_equally_distributed_sampling( length(q_with_equally_distributed_sampling)-floor(steady_state_behavior/uniform_step_size):end );
            t_steady_state = t_with_equally_distributed_sampling( length(t_with_equally_distributed_sampling)-floor(steady_state_behavior/uniform_step_size):end );
            ...
            A2 = sqrt(2)*sqrt( sum(q2_ss_centered) )*sqrt(uniform_step_size/steady_state_behavior); 
...
Omega_opt_eq100(iter) = sqrt((omega_gK^2 * sqrt((g_K1^2 / g_K^2) + 1) + omega_gK1^2 * sqrt(2)) / (sqrt((g_K1^2 / g_K^2) + 1) + sqrt(2)));
